...
void Render(const Mesh& mesh) {
	...
	for (int i = 0; i < mesh.numTriangles; ++i) {
		vec3 normal = AsNormal(Cross(mesh.triangles[i].c - mesh.triangles[i].a, mesh.triangles[i].b - mesh.triangles[i].a));
...
void Render(const Plane& plane) {
	...
	vec3 n = AsNormal(plane.normal);
	float d = plane.distance;
	vec3 tx = AsNormal(right);
	vec3 ty = AsNormal(up);
...
void Render(const Triangle& triangle, bool oneSided) {
	...
	if (!oneSided) {
		...
		vec3 norm = AsNormal(Cross(ac, ab));
...
void Render(const AABB& aabb) {
	...
	glTranslatef(aabb.origin.x, aabb.origin.y, aabb.origin.z);
